    ...
    d = function() {
        var a = {
            ...
            parse: function(a) {
                ...
                for (;;) {
                    r = c[c.length - 1],
                    this.defaultActions[r] ? s = this.defaultActions[r] : (p == null && (p = o()), s = f[r] && f[r][p]);
                    ...
                    case 1:
                        ...
                        c.push(s[1]),
                        p = null,
                        q ? (p = q, q = null) : (i = this.lexer.yyleng, g = this.lexer.yytext, h = this.lexer.yylineno, m = this.lexer.yylloc, j > 0 && j--);
                        break;
                    case 2:
                        ...
                        x && (c = c.slice(0, -1 * x * 2), d = d.slice(0, -1 * x), e = e.slice(0, -1 * x)),
        ...
        f = function() {
            ...
            a.rules = a.rules=[/^\s+/,/^-?([0-9]|[1-9][0-9]+)(\.[0-9]+)?([eE][-+]?[0-9]+)?\b/,/^"(\\["bfnrt/\\]|\\u[a-fA-F0-9]{4}|[^\0-\x09\x0a-\x1f"\\])*"/,/^\{/,/^\}/,/^\[/,/^\]/,/^,/,/^:/,/^true\b/,/^false\b/,/^null\b/,/^$/,/^./],
            a.conditions = {
                INITIAL: {
...
function validateJson(input_id) {
	var reformat = true, compress = false, lineNum, lineMatches, lineStart, lineEnd, jsonVal, result;
	var jsonObject = _namespcePage().find('#' + input_id);
	var jsonResult = _namespcePage().find("#" + input_id + "_results");
	jsonVal = jsonObject.val();
	try {
		result = jsl.parser.parse(jsonVal);
		if (result) {
			jsonResult.show().removeClass("error").addClass("success");
			// $('div.linedwrap').removeClass('redBorder').addClass('greenBorder');
			jsonResult.text('Valid JSON');
			if (reformat) {
				jsonObject.val(JSON.stringify(JSON.parse(jsonVal), null, "    "))
			}
			if (compress) {
				jsonObject.val(JSON.stringify(JSON.parse(jsonVal), null, ""))
			}
		...
		try {
			if (reformat) {
				jsonVal = jsl.format.formatJson(jsonObject.val());
				jsonObject.val(jsonVal);
				result = jsl.parser.parse(jsonObject.val())
			}
		} catch (e) {
			parseException = e
		}
		...
		if (lineMatches && typeof lineMatches === "object" && lineMatches.length > 1) {
			...
			jsonObject.focus().caret(lineStart, lineEnd)
		}
		jsonResult.text(parseException.message);
		jsonResult.show().removeClass('success').addClass('error');
	}
	return false;
}
function getNthPos(searchStr, char, pos) {
	var i, charCount = 0, strArr = searchStr.split(char);
	if (pos === 0) {
		return 0
	}
	for (i = 0; i < pos; i++) {
